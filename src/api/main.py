...
# Load environment variables, allowing .env to override existing shell values
...
async def query(
    ...
    try:
        client_host = None
        try:
            client_host = req.client.host if getattr(req, 'client', None) else 'unknown'
        except Exception:
            client_host = 'unknown'
        logger.info(f"Incoming query from {client_host}: {request.question[:200]}")
...
async def debug_search(q: str, db = Depends(get_db)):
    """Debug BM25 search endpoint for verifying retrieval."""
    from src.services.retrieval import get_document_retriever
    retriever = get_document_retriever()
    return retriever.retrieve(query=q, top_k=5, db=db)
...
async def startup_event():
    ...
    try:
        from src.services.retrieval import load_bm25_index, is_bm25_loaded
        load_bm25_index()
        if is_bm25_loaded():
            logger.info("BM25 search ready")
        else:
            logger.warning("BM25 index not loaded at startup")
    except Exception as e:
        logger.warning(f"⚠ BM25 index startup load failed: {e}")

    try:
        # Initialize vector store without expensive blocking collection count
        from src.services.vectorstore import get_vector_store
        vector_store = get_vector_store()
        logger.info(f"✓ Vector store initialized. Collection: {vector_store.collection_name}")

        # Delay expensive collection inspection until on-demand operations
        logger.info("⚡ Skipping ChromaDB collection count during startup to prevent startup hang")

        # Quick startup safeguard: finish startup early to avoid any further blocking
        logger.info("Startup complete (non-blocking checks skipped)")
        return
    ...
    try:
        # Check collection count and ingest PDFs if empty (in background thread)
        from src.services.vectorstore import get_vector_store
        from pathlib import Path
        
        vector_store = get_vector_store()
        # Use non-blocking collection info to avoid hanging during startup
        collection_info = vector_store.get_collection_info()
        collection_count = collection_info.get("total_embeddings", 0) if collection_info else 0

        if collection_count == 0:
            logger.info("🔄 ChromaDB collection is empty. Starting PDF ingestion in background...")
            ...
            if documents_path.exists():
                ...
                logger.info(f"Files in documents folder: {os.listdir(documents_path)}")
                logger.info(f"PDF files found: {len(pdf_files)}")
                
                if pdf_files:
                    logger.info(f"📚 Found {len(pdf_files)} PDFs to ingest in background")
                    try:
                        from ingest_new_documents import DocumentIngester
                        
                        def background_ingestion():
                            """Run PDF ingestion in background thread"""
                            try:
                                logger.info("⏳ Background ingestion started")
                                ingester = DocumentIngester()
                                ingester.ingest_all()
                                logger.info("✓ Background PDF ingestion completed")
                            except Exception as e:
                                logger.error(f"✗ Background ingestion failed: {e}")
                        
                        # Start ingestion in daemon thread (won't block startup)
                        ingestion_thread = threading.Thread(
                            target=background_ingestion,
                            daemon=True,
                            name="PDFIngestionThread"
                        )
                        ingestion_thread.start()
                        logger.info("✓ Background ingestion thread started (not blocking startup)")
                    except Exception as e:
                        logger.warning(f"⚠ Could not start background ingestion: {e}")
                else:
                    logger.info("⚠ No PDF files found in documents folder")
            else:
                logger.warning(f"Documents folder does not exist at: {documents_path}")
        else:
            logger.info(f"✓ ChromaDB collection ready ({collection_count} chunks indexed)")
    
    except Exception as e:
        logger.warning(f"⚠ Collection status check failed: {e}")
