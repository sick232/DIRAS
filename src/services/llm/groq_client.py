...
class GroqClient:
    ...
    def _is_model_decommissioned_error(self, error: Exception) -> bool:
        ...
        return (
            ...
            or ("invalid request parameters" in error_str and "model" in error_str)
    ...
    def _validate_api_key(self) -> bool:
        ...
        try:
            ...
            if self._is_model_decommissioned_error(e):
                logger.warning(f"⚠ Groq model '{self.model}' is unavailable: {str(e)}")
                return self._try_alternate_models()
            logger.warning(f"⚠ Groq API key validation failed: {str(e)}")
            return False

    def _try_alternate_models(self) -> bool:
        alternate_models = [
        ...
        for alt_model in alternate_models:
            if alt_model == self.model:
                continue
            try:
                logger.info(f"Trying alternate Groq model: {alt_model}")
                self._test_model(alt_model)
                self.model = alt_model
                logger.info(f"✓ Switched Groq model to {self.model}")
                return True
            except Exception as e:
                logger.warning(f"⚠ Alternate model {alt_model} failed: {str(e)}")
        logger.warning("⚠ No valid Groq model found during validation.")
        return False
